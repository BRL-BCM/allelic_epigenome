...
my $cpgPosFile = shift(@ARGV);
my $hetSnpPosFile = shift(@ARGV);
...
print "Chromosome\tStart\tEnd\tAllele1/Methylated\tAllele1/Unmethylated\tAllele2/Methylated\tAllele2/Unmethylated\tNumber.of.reads\tNumber.of.CpGs\tis.on.heterozygous.CpG\tAllele1\tAllele2\n";
while(my $snpLine = <SNPS>){
  chomp($snpLine);
  if($snpLine =~ /^\s*$/){
    next;
  }
  my ($snpChr,$start,$snpPos,$al1,$al2) = split("\t",$snpLine);
  my $noNegAs = 0;
  my $noPosTs = 0;
  my $orAl1 = $al1;
  my $orAl2 = $al2;
  ...
  my $getHetSnpReads = sub {
	...
	unless(exists($surroundingCpgs{"$chr:$pos"})){
	    $surroundingCpgs{"$chr:$pos"} = 0;
	}
	if(($qbase=~ /C/i && $strand == 1 && $cStrand eq "+") or ($qbase=~ /G/i && $strand == -1 && $cStrand eq "-")){
	  $reads{$readName}->[2]++;
	  $surroundingCpgs{"$chr:$pos"}++;
	}
  ...
  for my $snpRead (values(%reads)){
    ...
    if(($snpRead->[0] > 0 or $snpRead->[1] > 0)){
      $allelicMethTab[4]++;
    }
  }
  $allelicMethTab[5] = scalar(keys(%surroundingCpgs));
  
  print $snpChr,"\t",$snpPos-1,"\t",$snpPos,"\t",$allelicMethTab[0],"\t",$allelicMethTab[1],"\t",$allelicMethTab[2],"\t",$allelicMethTab[3],"\t",$allelicMethTab[4],"\t",$allelicMethTab[5],"\t",$orAl1,"\t",$orAl2,"\n";
  $processedSnps++;
